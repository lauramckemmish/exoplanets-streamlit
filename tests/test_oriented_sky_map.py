...
class SkyMapTests(unittest.TestCase):
    ...
    def test_planet_shopping_requires_a_deliberate_transition_before_showing_the_map(self):
        ...
        self.assertIn('with st.container(width=1200):', shopping_source)
    ...
    def test_planet_shopping_places_the_claim_boundary_before_post_choice_material(self):
        shopping_source = Path("experiences/planet_shopping.py").read_text()

        confirmation_index = shopping_source.index('st.success(f"Destination chosen: {destination_name}")')
        claim_boundary_call_index = shopping_source.index("hard_reveal(", confirmation_index)
        claim_boundary_prompt_index = shopping_source.index(
            "**You’ve chosen a promising candidate. Are we done?**", claim_boundary_call_index
        )
        claim_boundary_key_index = shopping_source.index(
            "_HABITABILITY_BOUNDARY_REVEAL_KEY", claim_boundary_call_index
        )
        layout_index = shopping_source.index(
            'map_column, interpretation_column = st.columns(', confirmation_index
        )
        ...
        completion_gate_index = shopping_source.rindex("completion_gate(selected)")

        self.assertLess(confirmation_index, claim_boundary_call_index)
        self.assertLess(claim_boundary_call_index, claim_boundary_prompt_index)
        self.assertLess(claim_boundary_call_index, claim_boundary_key_index)
        self.assertLess(claim_boundary_key_index, layout_index)
        self.assertLess(claim_boundary_key_index, unavailable_map_index)
        self.assertLess(layout_index, coda_index)
        self.assertLess(unavailable_map_index, completion_gate_index)
