...
class UNSWVisualSystemTests(unittest.TestCase):
    ...
    def test_shared_interaction_grammar_has_semantic_surfaces(self):
        ...
        self.assertIn("def notice_prompt", helpers)
        self.assertIn("def self_check", helpers)
        self.assertNotIn("def think_q", helpers)
        self.assertIn("interaction-marker'>REVEAL", helpers)
        self.assertIn('.st-key-notice_prompt', styles)
        self.assertNotIn('.st-key-think_q', styles)
