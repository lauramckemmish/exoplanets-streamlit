...
def custom_candidates(data: pd.DataFrame, stars: int, planet_rule: str, planets: int, radius: tuple[float, float], temperature: tuple[int, int] | None, max_distance: float | None) -> tuple[pd.DataFrame, pd.DataFrame]:
    current = data.copy()
