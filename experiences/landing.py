...
def _thumbnail_container_key(entry, button_key_prefix):
    """Return a safe, stable Streamlit key for a thumbnail card."""
    identity = f"{button_key_prefix}-{entry['name']}".lower()
    safe_identity = "".join(character if character.isalnum() else "-" for character in identity)
    return f"thumbnail-{safe_identity.strip('-')}"
    ...
    def render_card(entry):
        with st.container(border=True):
            ...
            if thumbnail:
                ...
                thumbnail_key = _thumbnail_container_key(entry, button_key_prefix)
                st.markdown(
                    f"<style>.st-key-{thumbnail_key} "
                    "{ max-width: 30rem; width: 100%; margin: 0 auto; }</style>",
