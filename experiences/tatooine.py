...
Learners turn a qualitative planet idea into dataset variables and explicit filtering rules, then judge candidates using recorded, conflicting and missing evidence.

**Useful facilitation stance**

Use the worked examples to make the translation from idea → variable → rule visible before learners construct or adjust their own criteria. Keep asking what each rule assumes, and distinguish an analytical choice from direct evidence about a planet.

**What to listen for**

Learners distinguishing:

- a criterion chosen for the investigation;
- evidence actually recorded about a planet; and
- missing or unknown information.

Unknown data are neither positive evidence for a match nor evidence that the physical criterion failed.
""".strip()
...
def prepare_page():
    ...
    if facilitator_notes_enabled():
        facilitator_preparation(
            FACILITATOR_ORIENTATION,
            key="tatooine_orientation",
            title="For facilitators — Find Your Perfect Planet",
        )
